...
import (
	...
	"github.com/sekibomazic/messanger/internal/core/domain"
)

// implements MessengerRepository interface
...
func (m *MessengerPostgresRepository) SaveMessage(message domain.Message) error {
	...
	if req.RowsAffected == 0 {
		return fmt.Errorf(fmt.Sprintf("messages not saved: %v", req.Error))
	}
	return nil
}
...
func (m *MessengerPostgresRepository) ReadMessages() ([]*domain.Message, error) {
	...
	if req.Error != nil {
		return nil, fmt.Errorf(fmt.Sprintf("messages not found: %v", req.Error))
	}
	return messages, nil
}
